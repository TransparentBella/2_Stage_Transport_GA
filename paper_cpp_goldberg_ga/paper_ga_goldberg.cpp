    ...
    void solve(const string& out_dir) {
        ...
        for (int gen = start_generation; gen <= cfg.generations; ++gen) {
            ...
            if (cur_best + 1e-9 < best_score) {
                best = intensify(pop[0]);
                best_score = evaluate(best);
                pop[0] = best;
            }

            if (gen % cfg.elitism_interval == 0) {
                for (int i = 0; i < static_cast<int>(pop.size()); ++i) {
                    pop[i] = intensify(pop[i]);
                    double sc = evaluate(pop[i]);
                    if (sc + 1e-9 < best_score) {
                        best_score = sc;
                        best = pop[i];
                    }
                    log_progress(gen, "elitism " + to_string(i + 1) + "/" + to_string(pop.size()));
                }
            }
...
private:
    ...
    double evaluate_uncached(const vector<int>& chrom) {
        ...
        for (int i = 0; i < inst.n_plants; ++i) {
            mcf.add_edge(source, plant_start + i, chrom[i] ? llround(inst.plant_cap[i] * cfg.flow_scale) : 0, 0);
            for (int j = 0; j < inst.n_depots; ++j) {
                mcf.add_edge(plant_start + i, depot_in_start + j, total, llround(inst.c[i][j] * cfg.cost_scale));
            }
        }
        for (int j = 0; j < inst.n_depots; ++j) {
            mcf.add_edge(depot_in_start + j, depot_out_start + j, chrom[inst.n_plants + j] ? llround(inst.depot_cap[j] * cfg.flow_scale) : 0, 0);
    ...
    vector<int> ls1(vector<int> current) {
    ...
    vector<int> ls2(vector<int> current) {
        double current_cost = evaluate(current);
        bool improved = true;
        while (improved) {
            improved = false;
    ...
    vector<int> intensify(const vector<int>& chrom) {
        return ls2(ls1(chrom));
    }
};

static Config parse_config(int argc, char** argv, string& instance_dir, string& out_dir, string& eval_chromosome) {
    Config cfg;
    ...
    if (out_dir.empty() && eval_chromosome.empty()) throw runtime_error("--out-dir is required");
    return cfg;
}

int main(int argc, char** argv) {
    try {
        string instance_dir, out_dir, eval_chromosome;
        Config cfg = parse_config(argc, argv, instance_dir, out_dir, eval_chromosome);
