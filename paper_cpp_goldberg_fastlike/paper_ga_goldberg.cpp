...
public:
    ...
    void solve(const string& out_dir) {
        ...
        if (cfg.resume && fs::exists(checkpoint_path)) {
            ...
            vector<int> all_open(inst.n_plants + inst.n_depots, 1);
            pop.push_back(all_open);
        ...
        for (int gen = start_generation; gen <= cfg.generations; ++gen) {
            ...
            if (cur_best + 1e-9 < best_score) {
                best = local_search_once(pop[0]);
                best_score = evaluate(best);
                pop[0] = best;
            }

            if (gen % cfg.elitism_interval == 0) {
                best = local_search_once(best);
                best_score = evaluate(best);
                pop[0] = best;
                log_progress(gen, "best-local-search");
            }
...
private:
    ...
    vector<int> local_search_once(const vector<int>& chrom) {
